...
import Link from "next/link";
import useSubtitle from "@/hooks/subs";
import { PlayIcon } from "@/components/icons/play-pause";
function getRootDomain(url: string) {
  try {
    const hostname = new URL(url).hostname;
...
export default function Player() {
  ...
  return (
    ...
    >
      ...
      {/* Loading tip */}
      {!playback.canPlay && startPlayer && <DynamicTip />}
      ...
      {!startPlayer && (
        <button
          ...
          aria-label="Play"
          className="group absolute left-1/2 top-1/2 z-50 -translate-x-1/2 -translate-y-1/2 cursor-pointer"
        >
          {/* Glow */}
          <span className="absolute inset-0 -z-10 size-28 rounded-full bg-white/20 blur-3xl transition-all duration-500 group-hover:scale-150 group-hover:bg-white/30" />

          {/* Button */}
          <span className="flex size-24 items-center justify-center rounded-full border border-white/30 bg-white/15 shadow-[0_8px_40px_rgba(0,0,0,0.4)] backdrop-blur-xl transition-all duration-300 group-hover:scale-110 group-hover:bg-white group-active:scale-95">
            <PlayIcon className="ml-1 size-10 fill-white text-white transition-colors duration-300 group-hover:fill-black group-hover:text-black" />
          </span>
        </button>
      )}
      {/* Server picker */}
      <AnimatePresence>
        {(isVisible || !playback.canPlay) && showServer && startPlayer && (
          <LyricsServerPicker
            servers={servers}
            playingIndex={playingIndex}
            activeIndex={serverIndex}
